...
static constexpr int itr_count = 3;
...
TestDF_bandwidth::run(std::shared_ptr<xrt_core::device> dev)
{
  ...
  const auto xclbin_name = xrt_core::device_query<xrt_core::query::xclbin_name>(dev, xrt_core::query::xclbin_name::type::df_bw);
 ...
 working_dev.register_xclbin(xclbin);
  ...
  try {
    sequence = XBValidateUtils::dpu_or_elf(dev, xclbin, ptree, xrt_core::query::elf_name::type::df_bw);
  ...
  if (sequence.find(".elf") == std::string::npos) { // run DPU sequence
    ...
    xrt::bo bo_instr(working_dev, instr_size*sizeof(int), XCL_BO_FLAGS_CACHEABLE, testker.group_id(5));
    ...
    for (int i = 0; i < itr_count; i++) {
      try {
        auto run = testker(host_app, bo_ifm, NULL, bo_ofm, NULL, bo_instr, instr_size, NULL);
    ...
    xrt::bo bo_ifm = xrt::bo{working_dev, rw_size, xrt::bo::flags::cacheable, 0};
    xrt::bo bo_ofm = xrt::bo{working_dev, rw_size, xrt::bo::flags::cacheable, 0};

    // map input buffer
    auto ifm_mapped = bo_ifm.map<int*>();
    for (size_t i = 0; i < rw_size / sizeof(uint32_t); i++)
      ifm_mapped[i] = rand() % 4096;
    
    xrt::run run{testker};
    run.set_arg(0, bo_ifm);
    run.set_arg(1, bo_ofm);
